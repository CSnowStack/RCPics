...
public class AlbumBitmapCacheHelper {
    ...
    private static volatile AlbumBitmapCacheHelper instance = null;//单例
    private LruCache<String, Bitmap> cache;//缓存
    private ArrayList<String> currentShowString;
    Context mContext;
    ThreadPoolExecutor tpe;//线程池
    ...
    public Bitmap getBitmap(String path, int width, int height, AlbumBitmapCacheHelper.ILoadImageCallback callback, Object... objects) {
        ...
        if(bitmap != null) {
            Log.v("AlbumBitmapCacheHelper", "getBitmap from cache");
        } else {
    ...
    private Bitmap getBitmap(String path, int widthLimit, int heightLimit) throws OutOfMemoryError {
        ...
        try {
            ...
            if(widthLimit == 0 && heightLimit == 0) {
                sampleSize1 = this.computeScale(e, mContext.getResources().getDisplayMetrics().widthPixels,  mContext.getResources().getDisplayMetrics().widthPixels);
            } else {
                int matrix;
